...
from NABQR import simulate_wind_power_sde, run_nabqr_pipeline
...
corrected_ensembles, q_hat, actuals_out, beta_params, scores, AI_response = run_nabqr_pipeline(
    X = ensembles,
    actuals = actuals,
    training_size = 0.7,            # use 70% of data for LSTM training, 30% for testing
    epochs = 100,                    # train the LSTM for 50 epochs (for demonstration purposes)
    timesteps = [0, 1, 2, 6, 12, 24],  # lag indices for LSTM input (hours back in time)
    quantiles = [0.05, 0.5, 0.95],  # target quantiles to predict (5th, 50th, 95th percentiles)
    visualize = False,                # whether to generate plots of the results
